...
// import { useAppDispatch, useAppSelector } from "@/hooks/useRedux";import { get } from "react-native/Libraries/TurboModule/TurboModuleRegistry";
...
import { Position } from "../../../types";
import { usePositionsList } from "@/hooks/queries/usePositionsList";

export default function PositionListScreen() {
  const router = useRouter();
  const { data: positions, isLoading, isError, error, deletePosition } =
    usePositionsList();

  const handleDeletePress = (id: number) => {
    Alert.alert("Xác nhận", "Bạn có chắc chắn muốn xóa vị trí này?", [
      { text: "Hủy", style: "cancel" },
      {
        text: "Xóa",
        style: "destructive",
