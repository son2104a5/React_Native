...
import { getAuthToken } from "../../../constants/auth";
        ...
        options={{
          ...
          <TouchableOpacity
            ...
            onPress={async () => {
              ...
              if (!token || !postId) {
                Alert.alert("Lỗi", "Thiếu thông tin xác thực hoặc ID bài viết");
                return;
              }
              try {
                await Api.updateArticle(token, String(postId), { title, content });
                Alert.alert("Thành công", "Bài viết đã được cập nhật");
                router.back();
              } catch (e: any) {
                Alert.alert("Lỗi", e?.message || "Không thể cập nhật bài viết");
              }
            }}
          >
