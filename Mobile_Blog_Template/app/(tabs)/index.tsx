    ...
    const fetchData = async () => {
      ...
      try {
        const [articlesRes, categoriesRes] = await Promise.all([
          Api.getArticles({ page: 1, limit: 10 }),
          Api.getCategories(),
        ]);

        if (!isMounted) return;

        const mappedLatest: LatestPost[] = (articlesRes?.data || articlesRes || []).map((a: any) => ({
          id: String(a.id ?? a._id ?? a.articleId ?? a.slug ?? Math.random()),
          title: a.title ?? "",
          author: a.author?.name ?? a.authorName ?? "",
          authorAvatar: a.author?.avatar ?? a.authorAvatar ?? "https://i.pravatar.cc/100",
          image: a.thumbnail ?? a.image ?? a.coverImage ?? "https://picsum.photos/400/300",
          date: a.publishedAt ?? a.createdAt ?? "",
        }));

        const mappedCategories: Category[] = (categoriesRes?.data || categoriesRes || []).map((c: any) => ({
          name: c.name ?? c.title ?? "",
          icon: "pricetag-outline",
        }));

        setLatestPosts(mappedLatest);
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        {loading && <Text style={{ paddingHorizontal: 20 }}>Đang tải...</Text>}
        {error && (
          <Text style={{ paddingHorizontal: 20, color: "red" }}>{error}</Text>
        )}
